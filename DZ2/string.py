...
# 2017.03.08 01:48:01 checked. P.Rusanov
# Отлично!
def donuts(count):
    if count >= 10:
        r = 'Количество пончиков: много'
    else:
        r = 'Количество пончиков: '+ str(count)
    return r
...
# 2017.03.08 01:48:06 checked. P.Rusanov
# Отлично!
def both_ends(s):
    if len(s) < 2:
        return ''
...
# 2017.03.08 01:48:11 checked. P.Rusanov
# Отлично!
def fix_start(s):
...
# 2017.03.08 01:48:17 checked. P.Rusanov
# Отлично!
def mix_up(a, b):    
...
# 2017.03.08 01:48:22 checked. P.Rusanov
# Отлично!
def not_bad(s):
    # Покороче:  if -1 < s.find('не') < s.find('плох'):
    if s.find('не') != -1 and s.find('не') < s.find('плох'):
        return s[:s.find('не')] + 'хорош' + s[s.find('плох') + 4:]  
    return s
...
# 2017.03.08 01:49:00 checked. P.Rusanov
# Отлично!
def front_back(a, b):
    # Имеет смысл вынести len(b) // 2 и len(a) // 2 в отдельные переменную
